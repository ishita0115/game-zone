...
export default function MirrorMazePage() {
  ...
  const handleCellClick = useCallback((row: number, col: number) => {
    ...
    if (cell.content.type === "mirror" && cell.content.playerIndex === gameState.currentPlayer) {
      const ns = rotateMirror(gameState, row, col);
      if (ns) setGameState(ns);
    } else if (cell.content.type === "empty") {
      const ns = placeMirror(gameState, row, col);
      if (ns) setGameState(ns);
    }
  }, [gameState]);

  const handleSelectMirror = useCallback((type: MirrorType) => {
    if (!gameState) return;
  ...
  return (
    <div className="flex min-h-screen flex-col items-center px-4 py-4">
      ...
      <div className="flex flex-wrap items-start justify-center gap-6">
        ...
        <GamePanel state={gameState} onSelectMirror={handleSelectMirror} onRestart={() => setGameState(null)} />
      </div>
    </div>
  );
}
