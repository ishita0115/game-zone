        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-pink-500/20 bg-pink-500/5 p-5">
            ...
            <div className="space-y-2.5">
              {[
                ["🃏", "Tap a card to flip it and reveal the emoji"],
                ["🎯", "Flip a second card — try to find its match!"],
                ["✅", "Match = you score a point + bonus turn!"],
                ["🏆", "Most pairs matched wins the game!"],
              ].map(([icon, text], i) => (
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              {Array.from({ length: playerCount }).map((_, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white outline-none focus:border-pink-500"
                  />
                </div>
              ))}
            </div>
            ...
            <button
              ...
              className="w-full rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 py-3 text-base font-bold transition-all hover:scale-105 hover:shadow-lg hover:shadow-pink-500/25"
            >
              Start Playing!
            </button>
          </div>
